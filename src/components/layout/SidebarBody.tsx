...
export function SidebarBody({
  ...
  profile: Profile;
  /** chiamato dopo il click su un link, utile per chiudere il drawer mobile */
  onNavigate?: () => void;
}) {
  ...
  return (
    <div className="flex h-full flex-col cohere-surface-near-black">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-6">
        ...
        <ul className="space-y-0.5">
          {items.map((it) => <li key={it.href}><NavLink {...it} /></li>)}
        </ul>
        ...
        <RoleGuard ruolo={profile.ruolo} ruoli={["admin"]}>
          ...
          <ul className="space-y-0.5">
            {adminItems.map((it) => <li key={it.href}><NavLink {...it} /></li>)}
          </ul>
        </RoleGuard>
      </nav>
